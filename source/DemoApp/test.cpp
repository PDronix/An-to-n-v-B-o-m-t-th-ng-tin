...
const Color HEADER_BLUE = { 30, 100, 170, 255 }; // Màu xanh dương đậm cho Trang chủ     

Font appFont;
Texture2D logoHaUI;
// ĐÃ THÊM TAB 'HOME' VÀO ĐẦU TIÊN
enum Tab { HOME, PARAM_GEN, KEY_SIGN, VERIFY, THEORY };
...
struct BigInt {
    ...
    std::string toHex() const { char buf[30]; snprintf(buf, sizeof(buf), "%llX", val); return std::string(buf); }
};
...
namespace FileIOHelper {
    void hashFileAsync(std::string filepath) {
        ...
        if (!file.is_open()) { asyncStatus = -2; isProcessing = false; return; }
        std::streamsize size = file.tellg();
        if (size == 0) { asyncStatus = -1; isProcessing = false; return; }
        file.seekg(0, std::ios::beg);
        uint64 hash = 5381; char buffer[8192]; std::streamsize bytesRead = 0;
        while (file.read(buffer, sizeof(buffer)) || file.gcount() > 0) {
            std::streamsize count = file.gcount();
            for (int i = 0; i < count; i++) hash = ((hash << 5) + hash) + buffer[i];
            bytesRead += count; asyncProgress = (float)bytesRead / size;
        }
        asyncHashResult = (hash == 0 ? 1 : hash); asyncStatus = 1; isProcessing = false;
    }

    bool exportKeys(const DSAParams& p, const PrivKey& priv, const PubKey& pub, std::string& outPrivName, std::string& outPubName) {
        time_t now = time(0); tm* ltm = localtime(&now); char timeBuf[30];
        ...
        if (privFile.is_open()) { privFile << p.p.val << "\n" << p.q.val << "\n" << p.g.val << "\n" << priv.x.val; privFile.close(); } else return false;
        std::ofstream pubFile(outPubName);
        if (pubFile.is_open()) { pubFile << p.p.val << "\n" << p.q.val << "\n" << p.g.val << "\n" << pub.y.val; pubFile.close(); return true; }
        return false;
    }
    ...
    bool loadPublicKey(const std::string& path, DSAParams& p, PubKey& pub) {
        ...
        try {
            size_t pos;
            p.p.val = std::stoull(p_str, &pos); if(pos != p_str.length()) return false; 
    ...
    bool loadSignature(const std::string& path, Signature& sig) {
        std::ifstream file(path); if (!file.is_open()) return false;
        std::string r_str, s_str; if (!(file >> r_str >> s_str)) return false;
        try {
            size_t pos;
...
namespace DSA {
    DSAParams generateParams() {
        ...
        uint64 h = 2, g; while (true) { g = MathDSA::powerMod(h, (p - 1) / q, p); if (g > 1) break; h++; }
        params.p = p; params.q = q; params.g = g; return params;
    }
    std::pair<PrivKey, PubKey> generateKeyPair(DSAParams p) {
        PrivKey priv; PubKey pub; priv.x = 1 + rand() % (p.q.val - 1);
    ...
    Signature signHash(uint64 hm, PrivKey key, DSAParams p) {
        ...
        do {
            ...
            uint64 hm_xr = (hm + xr) % p.q.val; s = MathDSA::mulMod(k_inv, hm_xr, p.q.val);
        } while (s == 0);
    ...
    bool verifyHash(uint64 hm, Signature sig, PubKey key, DSAParams p) {
        ...
        uint64 u1 = MathDSA::mulMod(hm, w, p.q.val); uint64 u2 = MathDSA::mulMod(r, w, p.q.val);
        uint64 gu1 = MathDSA::powerMod(p.g.val, u1, p.p.val); uint64 yu2 = MathDSA::powerMod(key.y.val, u2, p.p.val);
        uint64 v = (MathDSA::mulMod(gu1, yu2, p.p.val)) % p.q.val; return v == r;
    }
}
...
void DrawTextBox(int x, int y, int width, int height, const std::string& label, const std::string& text, Color textColor) {
    ...
    DrawRectangle(x, y, width, height, CARD_BG); DrawRectangleLines(x, y, width, height, BORDER_COLOR);
...
int main() {
    ...
    Tab currentTab = HOME; // Mở phần mềm lên sẽ hiển thị Trang Chủ
    std::string statusMessage = "Hệ thống sẵn sàng. Vui lòng làm theo quy trình ở Trang chủ.";
    Color statusColor = GREEN;

    while (!WindowShouldClose()) {
        if (IsFileDropped()) {
            FilePathList droppedFiles = LoadDroppedFiles(); std::string path = droppedFiles.paths[0]; 
            if (currentTab == KEY_SIGN && !isProcessing) {
                gState.fileToSignPath = path; statusMessage = "Đã tải file thành công. Sẵn sàng ký!"; statusColor = BLUE;
            } 
        ...
        if (!isProcessing) { 
            // Đã bổ sung Nút Tab Trang chủ và đẩy tọa độ Y của các nút xuống
            if (DrawModernButton(15, 100, 250, 45, "0. Giao Diện (Trang chủ)", currentTab == HOME)) currentTab = HOME;
            if (DrawModernButton(15, 160, 250, 45, "1. Tham số Hệ thống", currentTab == PARAM_GEN)) currentTab = PARAM_GEN;
            if (DrawModernButton(15, 220, 250, 45, "2. Quản lý Khóa & Ký", currentTab == KEY_SIGN)) currentTab = KEY_SIGN;
            if (DrawModernButton(15, 280, 250, 45, "3. Xác minh Chữ ký", currentTab == VERIFY)) currentTab = VERIFY;
            if (DrawModernButton(15, 340, 250, 45, "4. Lý thuyết Cơ chế", currentTab == THEORY)) currentTab = THEORY;
        }
        ...
        switch (currentTab) {
            ...
            case HOME: {
                // Khung Header Xanh đậm
                DrawRectangle(280, 0, 920, 150, HEADER_BLUE);
                DrawTextBeautiful("DSA - Digital Signature Algorithm", 320, 30, 32, WHITE);
                DrawTextBeautiful("Minh họa Chữ Ký Số - Nhóm 10 - HaUI-SICT-IT6001.2", 320, 80, 18, WHITE);
                DrawTextBeautiful("Chuẩn FIPS 186 | Khóa 64-bit (Mô phỏng) | Hàm băm Stream", 320, 110, 15, {200, 220, 255, 255});

                DrawTextBeautiful("QUY TRÌNH SỬ DỤNG", 320, 180, 18, HEADER_BLUE);

                // Thẻ Hướng dẫn 1
                DrawRectangle(320, 220, 840, 65, CARD_BG); DrawRectangleLines(320, 220, 840, 65, BORDER_COLOR);
                DrawRectangle(340, 232, 40, 40, HEADER_BLUE); DrawTextBeautiful("1", 353, 243, 20, WHITE);
                DrawTextBeautiful("Tab 1 - Tham số Hệ thống", 400, 230, 16, HEADER_BLUE);
                DrawTextBeautiful("Bấm nút để sinh tham số toán học cốt lõi của hệ thống DSA (p, q, g).", 400, 255, 15, TEXT_MAIN);

                // Thẻ Hướng dẫn 2
                DrawRectangle(320, 300, 840, 65, CARD_BG); DrawRectangleLines(320, 300, 840, 65, BORDER_COLOR);
                DrawRectangle(340, 312, 40, 40, HEADER_BLUE); DrawTextBeautiful("2", 353, 323, 20, WHITE);
                DrawTextBeautiful("Tab 2 - Quản lý Khóa & Ký File", 400, 310, 16, HEADER_BLUE);
                DrawTextBeautiful("Sinh cặp khóa (Bí mật/Công khai). Kéo thả file dữ liệu vào để tạo file chữ ký (.sig).", 400, 335, 15, TEXT_MAIN);

                // Thẻ Hướng dẫn 3
                DrawRectangle(320, 380, 840, 65, CARD_BG); DrawRectangleLines(320, 380, 840, 65, BORDER_COLOR);
                DrawRectangle(340, 392, 40, 40, HEADER_BLUE); DrawTextBeautiful("3", 353, 403, 20, WHITE);
                DrawTextBeautiful("Tab 3 - Xác minh Chữ ký", 400, 390, 16, HEADER_BLUE);
                DrawTextBeautiful("Thả 3 file (Gốc, .sig, public.key) để phần mềm đối chiếu tính toàn vẹn của dữ liệu.", 400, 415, 15, TEXT_MAIN);

                // Khu vực Công thức Toán học
                DrawTextBeautiful("CÔNG THỨC TOÁN HỌC LÕI DSA", 320, 480, 18, HEADER_BLUE);
                
                // Box Trái (Ký số)
                DrawRectangle(320, 520, 410, 180, {245, 250, 255, 255}); DrawRectangleLines(320, 520, 410, 180, {200, 220, 240, 255});
                DrawTextBeautiful("Ký số (Signing)", 340, 535, 16, HEADER_BLUE);
                DrawTextBeautiful("r = (g^k mod p) mod q", 340, 570, 15, TEXT_MAIN);
                DrawTextBeautiful("s = k^-1 * (H(m) + x*r) mod q", 340, 600, 15, TEXT_MAIN);
                DrawTextBeautiful("Điều kiện: 0 < k < q (Ngẫu nhiên và Bí mật)", 340, 650, 14, GRAY);

                // Box Phải (Xác minh)
                DrawRectangle(750, 520, 410, 180, {245, 250, 255, 255}); DrawRectangleLines(750, 520, 410, 180, {200, 220, 240, 255});
                DrawTextBeautiful("Xác thực (Verification)", 770, 535, 16, HEADER_BLUE);
                DrawTextBeautiful("w = s^-1 mod q", 770, 570, 15, TEXT_MAIN);
                DrawTextBeautiful("u1 = H(m)*w mod q   |   u2 = r*w mod q", 770, 600, 15, TEXT_MAIN);
                DrawTextBeautiful("v = (g^u1 * y^u2 mod p) mod q", 770, 630, 15, TEXT_MAIN);
                DrawTextBeautiful("Chữ ký hợp lệ khi: v == r", 770, 665, 16, {40, 150, 80, 255});
                break;
            }
            ...
            case KEY_SIGN: {
                ...
                if (gState.hasKeys && DrawModernButton(530, 80, 200, 40, "LƯU KHÓA (.KEY)", false, {230, 160, 50, 255})) {
                    ...
                    if (FileIOHelper::exportKeys(gState.params, gState.privKey, gState.pubKey, generatedPrivName, generatedPubName)) {
                        ...
                        statusMessage = "LỖI: Không thể ghi file khóa ra ổ cứng!"; statusColor = RED;
                    }
                }
                ...
                if (gState.hasKeys && !gState.fileToSignPath.empty()) {
                    if (!isProcessing && asyncStatus == 0) {
                        ...
                        if (asyncStatus == -1) {
                            ...
                            gState.signature = DSA::signHash(asyncHashResult, gState.privKey, gState.params); gState.hasSignature = true;
            ...
            case VERIFY: {
                ...
                DrawRectangle(310, 430, 850, 280, CARD_BG); DrawRectangleLines(310, 430, 850, 280, BORDER_COLOR);
            ...
            case THEORY: {
                DrawTextBeautiful("TÓM TẮT LÝ THUYẾT & CƠ CHẾ HOẠT ĐỘNG CỦA DSA", 310, 30, 22, TEXT_HEADING);

                // Section 1: Tổng quan
                DrawTextBeautiful("1. Tổng quan về DSA (Digital Signature Algorithm)", 310, 70, 18, HEADER_BLUE);
                DrawTextBeautiful("- Tiêu chuẩn chữ ký số do NIST đề xuất năm 1991 (Chuẩn FIPS 186).", 330, 100, 15, TEXT_MAIN);
                DrawTextBeautiful("- Độ an toàn dựa trên độ khó của bài toán Logarit rời rạc (Discrete Logarithm).", 330, 125, 15, TEXT_MAIN);
                DrawTextBeautiful("- DSA chỉ hỗ trợ Ký số và Xác thực, KHÔNG có chức năng mã hóa văn bản như RSA.", 330, 150, 15, TEXT_MAIN);

                // Section 2: Ba giai đoạn cốt lõi
                DrawTextBeautiful("2. Ba giai đoạn cốt lõi của thuật toán", 310, 190, 18, HEADER_BLUE);
                
                // Khung Tạo tham số & Khóa
                DrawRectangle(310, 225, 850, 85, CARD_BG); 
                DrawRectangleLines(310, 225, 850, 85, BORDER_COLOR);
                DrawTextBeautiful("[Giai đoạn 1] - Khởi tạo Hệ thống & Cặp Khóa", 325, 235, 16, TEXT_HEADING);
                DrawTextBeautiful("- Hệ thống chọn 3 số: p (số nguyên tố lớn), q (ước của p-1), g (phần tử sinh).", 345, 260, 15, TEXT_MAIN);
                DrawTextBeautiful("- Tạo Khóa bí mật (x): Chọn ngẫu nhiên x < q.  |  Tạo Khóa công khai (y): y = g^x mod p.", 345, 285, 15, TEXT_MAIN);

                // Khung Ký số
                DrawRectangle(310, 320, 850, 105, CARD_BG); 
                DrawRectangleLines(310, 320, 850, 105, BORDER_COLOR);
                DrawTextBeautiful("[Giai đoạn 2] - Quá trình Ký dữ liệu (Tạo ra r và s)", 325, 330, 16, TEXT_HEADING);
                DrawTextBeautiful("- Băm file (Hash) để lấy chuỗi đại diện H(m). Chọn số ngẫu nhiên k bí mật (0 < k < q).", 345, 355, 15, TEXT_MAIN);
                DrawTextBeautiful("- Tính r = (g^k mod p) mod q.", 345, 380, 15, TEXT_MAIN);
                DrawTextBeautiful("- Tính s = [k^-1 * (H(m) + x*r)] mod q. Chữ ký xuất ra là cặp giá trị (r, s).", 345, 405, 15, TEXT_MAIN);

                // Khung Xác minh
                DrawRectangle(310, 435, 850, 105, CARD_BG); 
                DrawRectangleLines(310, 435, 850, 105, BORDER_COLOR);
                DrawTextBeautiful("[Giai đoạn 3] - Quá trình Xác minh (Dùng Khóa công khai y)", 325, 445, 16, TEXT_HEADING);
                DrawTextBeautiful("- Băm lại file nhận được để tính H(m)'.", 345, 470, 15, TEXT_MAIN);
                DrawTextBeautiful("- Tính w = s^-1 mod q  ->  u1 = H(m)' * w mod q  ->  u2 = r * w mod q.", 345, 495, 15, TEXT_MAIN);
                DrawTextBeautiful("- Tính v = [(g^u1 * y^u2) mod p] mod q. Nếu v == r => Chữ ký hợp lệ!", 345, 520, 15, TEXT_MAIN);

                // Section 3: Đặc tính bảo mật
                DrawTextBeautiful("3. Đặc tính bảo mật", 310, 560, 18, HEADER_BLUE);
                DrawTextBeautiful("- Tính Toàn vẹn: Sửa 1 byte file gốc -> H(m) đổi -> v khác r -> Phát hiện giả mạo.", 330, 590, 15, TEXT_MAIN);
                DrawTextBeautiful("- Chống chối bỏ: Chỉ người giữ Khóa bí mật (x) mới tạo ra được chữ ký (r,s).", 330, 615, 15, TEXT_MAIN);
                DrawTextBeautiful("- LƯU Ý: Biến ngẫu nhiên k phải là duy nhất và giữ bí mật tuyệt đối cho mỗi lần ký.", 330, 640, 15, DANGER_COLOR);
    ...
    UnloadTexture(logoHaUI); UnloadFont(appFont); CloseWindow(); return 0;
}