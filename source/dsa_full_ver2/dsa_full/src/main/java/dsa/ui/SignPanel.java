...
public class SignPanel extends JPanel {
    ...
    private JPanel makeControlPanel() {
        ...
        textCard.setMaximumSize(new Dimension(Integer.MAX_VALUE, 120));
        ...
        tRow.add(btnSignText, BorderLayout.EAST);

        textCard.add(UIStyle.muted("Nhap noi dung can ky:"), BorderLayout.NORTH);
        ...
        // Info file + nút chọn
        JPanel fileInfo = new JPanel(new BorderLayout(6, 0)); fileInfo.setOpaque(false);
        ...
        JPanel sRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0)); sRow.setOpaque(false);
        sRow.add(btnSaveSig);
    ...
    private JPanel makeDropZone() {
        ...
        new DropTarget(zone, new DropTargetListener() {
            ...
            public void dragEnter(DropTargetDragEvent e)  { zone.setBackground(new Color(219, 234, 254)); }
            public void dragExit(DropTargetEvent e)       { zone.setBackground(new Color(239, 246, 255)); }
    ...
    private void signText() {
        ...
        if (text.isEmpty()) { warn("Vui long nhap noi dung can ky."); return; }

        // Hoi nguoi dung co muon luu van ban thanh file .txt khong
        int choice = JOptionPane.showConfirmDialog(this,
            "Ban co muon luu van ban nay thanh file .txt truoc khi ky?\n"
            + "(Nen luu de co the kiem tra lai sau)",
            "Luu Van Ban", JOptionPane.YES_NO_CANCEL_OPTION);
        if (choice == JOptionPane.CANCEL_OPTION) return;

        String sourceName = "van_ban";
        if (choice == JOptionPane.YES_OPTION) {
            JFileChooser fc = new JFileChooser();
            fc.setDialogTitle("Luu van ban thanh file .txt");
            fc.setSelectedFile(new File("van_ban.txt"));
            if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                try {
                    ...
                    lastSavedTextFile = f;
                    append("\n>>> Van ban da luu: " + f.getAbsolutePath());
                } catch (Exception ex) {
                    warn("Loi luu van ban: " + ex.getMessage());
                    return;
                }
            }
        }
    ...
    private void signFile() {
        if (!ctx.hasKeys()) { warnNoKey(); return; }
        if (selectedFile == null) { warn("Vui long chon file truoc!"); return; }
    ...
    private void saveSignature() {
        if (ctx.lastSignature == null) { warn("Chua co chu ky. Hay ky truoc!"); return; }
        JFileChooser fc = new JFileChooser();
